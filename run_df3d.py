...
datapath = 'data/deepfashion_rendering_fov60'
scenes = [30, 92, 117, 133, 164, 204, 300, 320, 448, 522, 591, 598]
MAX_WORKER = 8
target_name = 'release_df3d'
...
def train_block(gpu_id, scene):
    # training
    out_path = f"output_df/{current_date}-{target_name}/{scene}/"
    if not os.path.exists(f"{out_path}/chkpnt30000.pth"):
        cmd = f'python train_deepfashion.py -s data/deepfashion_rendering_fov60/{scene} --gpu {gpu_id} -m {target_name}/{scene} --lambda_dist 0 --outdir output_df'
        print(cmd)
        os.system(cmd)
...
def dispatch_jobs(jobs, executor):
    ...
    os.chdir('eval_dtu')
    cmd = f'python eval_deepfasion.py --target_name {current_date}-{target_name}'
    print(cmd)
    os.system(cmd)
    os.chdir('..')
