...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    first_iter = 0
    pc_bbx = torch.tensor([[-0.55, -0.55, -0.55], [0.55, 0.55, 0.55]]).cuda()
    tb_writer, tb_writer2 = prepare_output_and_logger(dataset)

    gaussians = GaussianModel(dataset.sh_degree)
    field = UDFNetwork(range_=pc_bbx, last_layer='relu').cuda()
    ...
    for batch in range(9_999_999):
        ...
        if rgb_flag:
            ...
            # loss
            rgb_total_loss = loss + dist_loss + normal_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[9000, 12000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[9000, 12000, 30_000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[9000, 12000, 30000])
