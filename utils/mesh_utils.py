...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            # import numpy as np
            # intrinsic = viewpoint_cam.K
            # extrinsic = viewpoint_cam.w2c
            # depth_pts = depth_to_world_pts(depth.unsqueeze(0), intrinsic.unsqueeze(0), extrinsic.unsqueeze(0))[0]
            # np.savetxt("tmp_results/0715depth_world_pts.xyz", depth_pts.detach().cpu().numpy())
            # exit()
    ...
    def extract_mesh_bounded(self, voxel_size=0.004, sdf_trunc=0.02, depth_trunc=3, mask_backgrond=True):
        ...
        for i, cam_o3d in tqdm(enumerate(to_cam_open3d(self.viewpoint_stack)), desc="TSDF integration progress"):
            ...
            # print(cam_o3d.intrinsic.shape, depth.shape, cam_o3d.extrinsic.shape)
            # exit()
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            depth = self.depthmaps[idx][0]
            depth = (depth - depth.min())/(depth.max()-depth.min())
            depth = depth.unsqueeze(-1).repeat(1,1,3)
            save_img_u8(depth.cpu().numpy(), os.path.join(vis_path, 'depth_{0:05d}'.format(idx) + ".png"))
