...
def get_udf_normals_grid_slow(func, func_grad, N=56, max_batch=int(2 ** 20), level=0.0, voxel_origin=[-0.55, -0.55, -0.55]):
    """
    ...
    voxel_size = 2*abs(voxel_origin[0]) / (N - 1)
    ...
    while head < num_samples:
        ...
        grad = func_grad(input).detach().cpu()
...
def get_udf_normals_grid_fast(func, func_grad, samples, indices, N=56, max_batch=int(2 ** 18),voxel_origin=[-0.55,-0.55,-0.55]):
    """
    ...
    if indices is None or samples is None:
        return get_udf_normals_grid_slow(func, func_grad, N, max_batch, voxel_origin=voxel_origin)
    # network.eval()
    ################
...
def get_mesh_udf_fast(func, func_grad, samples=None, indices=None, N_MC=128,
                      gradient=True, eps=0.1, border_gradients=False, smooth_borders=False, dist_threshold_ratio=1., level=0.0, voxel_origin=[-0.55, -0.55, -0.55]):
    """
    ...
    """
    ### 1: sample grid
    df_values, normals, samples = get_udf_normals_grid_fast(func, func_grad, samples, indices, N=N_MC, voxel_origin=voxel_origin)
    df_values[df_values < 0] = 0
    ### 2: run our custom MC on it
    N = df_values.shape[0]
    voxel_size = abs(voxel_origin[0])*2 / (N - 1)
    verts, faces, _, _ = udf_mc_lewiner(df_values.cpu().detach().numpy(),
                                        normals.cpu().detach().numpy(),
                                        spacing=[voxel_size] * 3, level=level)
    verts = verts + voxel_origin[0] # since voxel_origin = [-1, -1, -1]
